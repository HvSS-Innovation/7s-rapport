// Startar server + deny-proxy och kör härdat-svepet över alla sidor.
// Kör: node test/opsec/run-sweep.js
'use strict';
...
async function ensureFixture() {
    ...
    console.log('[svep] hämtar test-fixture…');
...
(async () => {
    ...
    const svep = spawn(process.execPath, [path.join(__dirname, 'sweep.js')], { stdio: 'inherit' });
    const kod = await new Promise(res => svep.on('exit', res));
    server.kill();
    proxy.kill();
    process.exit(kod === 0 ? 0 : 1);
})().catch(err => { console.error('[svep] FEL:', err.message); process.exit(1); });
