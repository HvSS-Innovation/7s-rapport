...
- **Aktiveringen verifierar nu att paketet går att LÄSA, inte bara att det finns.** `detectKind()` svalde alla fel vid header-läsningen och antog "vector" — lagret byggdes, varningsraden blev grön "Härdat läge PÅ", och varje tile misslyckades tyst. Nu är en oläsbar header ett misslyckande med rollback. Race-testet skärpt: oläsbart paket ska ge `activate() === false` och ett felläge (tidigare accepterades `true`).
- **Misslyckad boot-aktivering lämnar inte längre en grå karta med evig spinner.** OTM-lagret läggs medvetet aldrig på när härdat är lagrat, men bara en av fem felgrenar i `activate()` lade tillbaka det; boot körs dessutom tyst. Alla grenar går nu genom en gemensam återställning. **Fail-closed även i felvägen:** låg online-lagret inte på kartan när aktiveringen började (= boot i härdat) läggs det INTE tillbaka automatiskt — operatören trodde sig vara i härdat, och ett tyst OTM-lager kring senaste kartposition vore just den läcka spärren finns för. I stället: kartan lämnas tom, spinnern döljs och varningsraden blir röd med orsaken, "Försök igen" och en uttrycklig "Visa online-karta (nät)".
- **Felläget syns i alla kartor:** controllern exponerar `getFel()`/`visaNormal()`; varningsraden (kartmodalen på sex rapportsidor, MINKARTA, SENSORSKISS) renderar det. Nytt invariant-fall: sida utan SW-controller med lagrat härdat ska visa felläget, dölja spinnern och erbjuda båda knapparna — utan att en enda tile begärs.
- **Feltexten skyller inte längre på "Range + CORS".** Spärren är redan på när paketet läses, så orsaken är alltid lokal (cache, service worker, minne); texten säger det och föreslår att radera + hämta om paketet. Alert:en respekterar nu `quiet` vid boot. "Ladda om sidan" ersatt med "stäng och öppna appen igen (eller ladda om sidan)" — en installerad iOS-app har varken adressfält eller pull-to-refresh.
- **Header-knappen "HÄRDAT" i kartmodalen borttagen.** På telefonskärmen lästes den som en statusbricka "härdat är på" samtidigt som raden under sa att bakgrunden hämtas externt och erbjöd "Slå på Härdat läge" — två kontroller, två budskap. Varningsraden är nu enda ingången: orange + "Slå på" i oläge, grön + "Ändra…" (öppnar väljaren, där av-slag finns) i påläge, röd i felläge.
- **Landskaps-väljaren gick inte att använda på iPhone.** Kartan (52 vh, ej krympbar) och en trerads-footer lämnade ~30 px åt listan — bara "HELA LANDET" syntes, raderna med Nedladdad/Visa/Radera och grannländerna (som saknar geometri i kartan) gick inte att nå. Nu scrollar hela kroppen på mobil, footern är kompakt och `100dvh` + `env(safe-area-inset-bottom)` håller "Ladda ner"-knappen ovanför Safaris verktygsfält.
- Alla fyra testlager gröna: egress-gate, invariant (26/26), svep (78 sidladdningar, 0 egress), race.
- **Kvar, dokumenterat i roadmap-filen:** SW-delegerad nedladdning hänger om SW:n dör (iOS dödar den vid skärmlås); `swHardened()` saknar timeout; 3 s controller-väntan/1,5 s ACK är tighta på kall iOS-start; WebKit läser troligen hela paketet i minnet per tile-anrop (`cache.match` + `.blob()`) — testa Öland (14 MB) före ett stort landskap; minikartan under STÄLLE ringer OTM vid sidladdning i normalläge (inget beslut om borttagning finns — förslag: statisk lokal bild).
